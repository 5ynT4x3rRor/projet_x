...
void handle_commands() {
    ...
    // Aération : laisse un espace vide sous la bannière au lancement
    std::cout << "\n";
    ...
    while (true) {
        ...
        if (command == "exit" || command == "quit") {
            ...
            if (candidate.empty() || candidate.find(' ') != std::string::npos || candidate.length() > 253 || candidate.find('.') == std::string::npos) {
                ...
                if (!valid) {
                    std::cout << "\033[1;31m[-] Erreur : Caractères non autorisés dans la cible.\033[0m\n";
                } else {
                    target = candidate;
        ...
        else if (command.rfind("output normal ", 0) == 0) {
            std::string filename = trim(command.substr(14));
            if (filename.empty() || filename.find(' ') != std::string::npos) {
                std::cout << "\033[1;31m[-] Erreur : Nom de fichier invalide (ex: output normal scan.txt).\033[0m\n";
            } else {
                save_scan_report(filename, target, port_start, port_end, "normal");
            }
        }
